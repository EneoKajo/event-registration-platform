...
function AdminEditEvent({ from }) {
  ...
  useEffect(() => {
    if (auth) {
      axios.get(`http://localhost:8081/admin/event/${eventId}`)
        .then(response => {
          if (response.data.Status === 'Success') {
  ...
  const handleSubmit = (event) => {
    ...
    axios.post(`http://localhost:8081/admin/edit-event/${eventId}`, {
      name,
      description,
      date,
      location,
    ...
    <div className={style.content}>
      ...
      <div className={style.container}>
        <form onSubmit={handleSubmit}>
          ...
          <label className={style.formLabel}>
            Organizer:
            <input type="text" value={organizer} onChange={(e) => setOrganizer(e.target.value)} className={style.formControl} />
          </label>
          <br />
